<script lang="ts">
	...
	let copy: PageCopy = $derived({
		metaTitle: t("webmToMp4.meta.title"),
		metaDesc: t("webmToMp4.meta.desc"),
		heroTitle: t("webmToMp4.hero.title"),
		heroSub: t("webmToMp4.hero.sub"),
		pill1: t("webmToMp4.pill.webm"),
		pill2: t("convert.pill.noInstall"),
		pill3: t("hero.pill1"),
		dropTitle: t("webmToMp4.drop.title"),
		dropSub: t("convert.drop.sub"),
		browse: t("btn.browse"),
		hint: t("webmToMp4.hint"),
		formatLabel: t("convert.format.label"),
		convertButton: t("webmToMp4.btn.convert"),
		loadingLabel: t("convert.status.loading"),
		convertingLabel: t("webmToMp4.status.converting"),
		doneLabel: t("status.done"),
		keepOpen: t("convert.warning.keepOpen"),
		selectVideoError: t("webmToMp4.error.selectVideo"),
		resultTitle: t("webmToMp4.res.title"),
		resultSub: t("webmToMp4.res.sub"),
		download: t("webmToMp4.btn.download"),
		newFile: t("webmToMp4.btn.new"),
	...
	let relatedTools = $derived(getRelatedTools('webm-to-mp4', currentLangKey, t));
</script>
...
<main>
	<div class="wrap">
		...
		<div class="page-layout">
			...
			<!-- ── Tool column ── -->
			<div class="tool-col">
				<VideoConverter {copy} sampleVideoUrl="/file_example_WEBM_1920_3_7MB.webm"/>
					...
					<div class="faq-list">
						{#each Array.from({ length: 10 }, (_, i) => i + 1) as n}
							...
							>
								...
								>
									<span itemprop="text">
										{@html markdownToHtml(t(`faq.webmToMp4.${n}.a`))}
									</span>
								</div>
							</details>
						{/each}
