<script lang="ts">
	...
	let hasFaq = $derived(
		Array.from({ length: 8 }, (_, i) => i + 1).every(
			(n) =>
				Boolean(translations[currentLangKey]?.[`faq.wc.${n}.q`]) &&
				Boolean(translations[currentLangKey]?.[`faq.wc.${n}.a`]),
		),
	);
	...
	let relatedTools = $derived(getRelatedTools('word-counter', currentLangKey, t));
</script>
