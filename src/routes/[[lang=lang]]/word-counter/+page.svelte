<script lang="ts">
	...
	let copy: PageCopy = $derived({
		metaTitle:        t('wc.meta.title'),
		metaDesc:         t('wc.meta.desc'),
		heroTitle:        t('wc.hero.title'),
		heroSub:          t('wc.hero.sub'),
		pill1:            t('wc.pill.realtime'),
		pill2:            t('wc.pill.noUpload'),
		pill3:            t('wc.pill.free'),
		// Tool UI
	...
	let relatedTools = $derived(getRelatedTools('character-counter', currentLangKey, t));
</script>
...
<main>
	<div class="wrap">
		...
		<CharacterCounter {copy} {relatedTools} pageType="word-counter"/>
				...
				<div class="faq-list">
					{#each Array.from({ length: 8 }, (_, i) => i + 1) as n}
						...
						>
							...
							>
								<span itemprop="text">
									{@html markdownToHtml(t(`faq.wc.${n}.a`))}
								</span>
							</div>
						</details>
					{/each}
