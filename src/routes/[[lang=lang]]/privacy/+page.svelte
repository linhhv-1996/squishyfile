<script lang="ts">
    ...
    // Chỉ load các file privacy.md
    const privacyModules = import.meta.glob('$lib/contents/legal/*/privacy.md', { 
        query: '?raw', 
        import: 'default' 
    });
    ...
    $effect(() => {
        const loadContent = async () => {
            isLoading = true;
            try {
                const path = `/src/lib/contents/legal/${lang}/privacy.md`;
                
                if (privacyModules[path]) {
                    const mdText = await privacyModules[path]() as string;
                    htmlContent = await marked.parse(mdText);
                } else {
                    // Fallback cứng về tiếng Anh nếu không tìm thấy file ngôn ngữ hiện tại
                    const fallbackPath = `/src/lib/contents/legal/en/privacy.md`;
                    if (privacyModules[fallbackPath]) {
                        const mdText = await privacyModules[fallbackPath]() as string;
                        htmlContent = await marked.parse(mdText);
                    } else {
                        htmlContent = '<h1>Privacy Policy not found</h1>';
                    }
                }
            } catch (error) {
