<script lang="ts">
	...
	let copy: PageCopy = $derived({
		metaTitle:        t('mc.meta.title'),
		metaDesc:         t('mc.meta.desc'),
		heroTitle:        t('mc.hero.title'),
		heroSub:          t('mc.hero.sub'),
		pill1:            t('mc.pill.realtime'),
		pill2:            t('mc.pill.noUpload'),
		pill3:            t('mc.pill.free'),
		// Tool UI
	...
	let hasFaq = $derived(
		Array.from({ length: 8 }, (_, i) => i + 1).every(
			(n) =>
				Boolean(translations[currentLangKey]?.[`faq.cc.${n}.q`]) &&
				Boolean(translations[currentLangKey]?.[`faq.cc.${n}.a`]),
		),
	);
	...
	let relatedTools = $derived(getRelatedTools('manuscript-counter', currentLangKey, t));
</script>
...
<main>
	<div class="wrap">
		...
		<CharacterCounter {copy} {relatedTools} pageType="manuscript-counter"/>
				...
				<div class="faq-list">
					{#each Array.from({ length: 8 }, (_, i) => i + 1) as n}
						...
						>
							...
							>
								<span itemprop="text">
									{@html markdownToHtml(t(`faq.mc.${n}.a`))}
								</span>
							</div>
						</details>
					{/each}
