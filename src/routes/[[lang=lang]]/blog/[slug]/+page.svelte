<script lang="ts">
	...
	import { Clock, ArrowLeft, Home, Tag } from 'lucide-svelte';
...
<svelte:head>
	<title>{data.post.title} – SquishyFile Blog</title>
	...
	<div class="wrap">
		...
		<article class="article">
			...
			<div class="art-cta">
				...
				<a href={homeHref} class="cta-btn">Open SquishyFile →</a>
			</div>

			<div class="art-back">
				<a href={blogHref} class="back-link"><ArrowLeft size={15} strokeWidth={2.2} /> Back to all articles</a>
			</div>
		</article>
	</div>
</main>
...
<style>
	...
	.bc-cur { color: var(--text); overflow: hidden; text-overflow: ellipsis; white-space: nowrap; max-width: 280px; }
	...
	:global(.art-body h2) {
		...
		margin: 36px 0 12px; letter-spacing: -.3px; padding-bottom: 8px; border-bottom: 1px solid var(--border);
	}
	...
	:global(.art-body code) {
		font-family: 'JetBrains Mono', 'Fira Code', ui-monospace, monospace; font-size: 13px; padding: 2px 6px;
	...
	:global(.art-body .table-wrap) { overflow-x: auto; margin-bottom: 24px; border: 1px solid var(--border); border-radius: var(--r); }
	:global(.art-body table) { width: 100%; border-collapse: collapse; }
	:global(.art-body th) {
		background: var(--surf2); padding: 10px 14px; font-size: 12px; font-weight: 700;
		text-transform: uppercase; letter-spacing: .5px; color: var(--muted); text-align: left;
	}
	...
	.art-cta {
		margin: 48px 0 32px; background: linear-gradient(135deg, rgba(79,140,255,.08), rgba(124,92,252,.08));
