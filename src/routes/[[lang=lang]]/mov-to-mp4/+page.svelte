<script lang="ts">
	...
	let copy: PageCopy = $derived({
		metaTitle: t("movToMp4.meta.title"),
		metaDesc: t("movToMp4.meta.desc"),
		heroTitle: t("movToMp4.hero.title"),
		heroSub: t("movToMp4.hero.sub"),
		pill1: t("movToMp4.pill.mov"),
		pill2: t("convert.pill.noInstall"),
		pill3: t("hero.pill1"),
		dropTitle: t("movToMp4.drop.title"),
		dropSub: t("convert.drop.sub"),
		browse: t("btn.browse"),
		hint: t("movToMp4.hint"),
		formatLabel: t("convert.format.label"),
		convertButton: t("movToMp4.btn.convert"),
		loadingLabel: t("convert.status.loading"),
		convertingLabel: t("movToMp4.status.converting"),
		doneLabel: t("status.done"),
		keepOpen: t("convert.warning.keepOpen"),
		selectVideoError: t("movToMp4.error.selectVideo"),
		resultTitle: t("movToMp4.res.title"),
		resultSub: t("movToMp4.res.sub"),
		download: t("movToMp4.btn.download"),
		newFile: t("movToMp4.btn.new"),
	...
	let relatedTools = $derived(getRelatedTools('mov-to-mp4', currentLangKey, t));
</script>
...
<main>
	<div class="wrap">
		...
		<div class="page-layout">
			...
			<!-- ── Tool column ── -->
			<div class="tool-col">
				<VideoConverter {copy} sampleVideoUrl="/file_example_MOV_1920_2_2MB.mov"/>
					...
					<div class="faq-list">
						{#each Array.from({ length: 10 }, (_, i) => i + 1) as n}
							...
							>
								...
								>
									<span itemprop="text">
										{@html markdownToHtml(t(`faq.movToMp4.${n}.a`))}
									</span>
								</div>
							</details>
						{/each}
