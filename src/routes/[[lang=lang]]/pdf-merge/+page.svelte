<script lang="ts">
	...
	let relatedTools = $derived(
		getRelatedTools("pdf-merge", currentLangKey, t),
	);
	...
	async function loadSamplePdfs(event?: MouseEvent) {
		...
		try {
			...
			const files = blobs.map(
				(blob, i) =>
					new File([blob], `sample-document-${i + 1}.pdf`, {
	...
	async function generateThumbnail(item: PdfItem) {
		try {
			...
			pdfFiles = pdfFiles.map((f) =>
				f.id === item.id ? { ...f, thumbLoading: false } : f,
			);
		}
	}
	...
	function onCardDragStart(e: DragEvent, id: string) {
		...
		if (e.dataTransfer && e.target instanceof HTMLElement) {
			e.dataTransfer.setDragImage(
				e.target.closest(".p-file-card") as HTMLElement,
				60,
				60,
			);
		}
	...
	<div class="wrap">
		<section class="hero">
			...
			<div class="hero-pills">
				<div class="pill">
					<span class="pill-ico">✨</span>{t("pdfMerge.pill.free")}
				</div>
				<div class="pill">
					<span class="pill-ico">🔒</span>{t(
						"pdfMerge.pill.noUpload",
					)}
				</div>
				<div class="pill">
					<span class="pill-ico">⚡</span>{t("pdfMerge.pill.browser")}
				</div>
			</div>
		</section>
		...
		<div class="page-layout">
			...
			<div class="tool-col">
						...
						<div class="p-viewer-zone">
							<div class="p-viewer-scroll">
								<div
									...
									class="p-viewer-pages"
								></div>
								{#if viewerLoading}
									<div
										class="p-viewer-loading"
						...
						<div class="p-panel">
							<div class="p-result-head">
								...
								<div>
									<div class="p-result-title">
										{t("pdfMerge.res.title")}
									</div>
									<div class="p-result-stats">
										{pdfResult.files}
										{t("pdfMerge.stat.files")} · {pdfResult.output}
									</div>
								</div>
							</div>
							<div class="p-result-actions">
								...
								<button
									...
									onclick={clearFiles}
								>
									<FileText size={13} strokeWidth={2} />{t(
										"pdfMerge.btn.new",
									)}
								</button>
							</div>
						...
						<div class="p-grid-wrap">
							<div class="p-grid-header">
								<span class="p-grid-label">
									{pdfFiles.length} file{pdfFiles.length !== 1
										? "s"
										: ""}
									<span class="p-grid-hint"
										>— {t(
											"pdfMerge.hint.dragToReorder",
										)}</span
									>
								</span>
								<button
									...
									disabled={pdfBusy}
								>
									<FilePlus size={14} strokeWidth={2} />{t(
										"pdfMerge.btn.addFiles",
									)}
								</button>
							</div>
							...
							<div class="p-grid">
								{#each pdfFiles as item, index (item.id)}
									<div
										...
										class:drag-target={dragOverId ===
											item.id && dragSrcId !== item.id}
										data-card-id={item.id}
										draggable={!pdfBusy}
										ondragstart={(e) =>
											onCardDragStart(e, item.id)}
										ondragover={(e) =>
											onCardDragOver(e, item.id)}
										ondragleave={onCardDragLeave}
										ondrop={(e) => onCardDrop(e, item.id)}
										ondragend={onCardDragEnd}
										ontouchstart={(e) =>
											onCardTouchStart(e, item.id)}
										in:fade={{ duration: 140 }}
									>
										<!-- Thumbnail -->
										<div class="p-thumb">
											{#if item.thumbLoading}
												<div class="p-thumb-loading">
													<div
														class="p-spinner"
													></div>
												</div>
											...
											<!-- Order badge -->
											<div class="p-order-badge">
												{index + 1}
											</div>

											<!-- Pages badge -->
											{#if item.pageCount > 0}
												<div class="p-pages-badge">
													{item.pageCount}p
												</div>
											{/if}
								...
								{#if pdfProcessing}
									<div class="p-spinner-row">
										...
										<span class="p-spinner-label"
											>{t(
												"pdfMerge.status.merging",
											)}</span
										>
									</div>
									<p class="p-warning">
										{t("pdfMerge.warning.keepOpen")}
									</p>
								{/if}
								{#if pdfError}
									<div class="p-error">
				...
				<div class="pnote">
					<span class="ni"
						><ShieldCheck size={16} strokeWidth={2} /></span
					>
				...
				>
					...
					<div class="faq-list">
						{#each Array.from({ length: 13 }, (_, i) => i + 1) as n}
							<details
								class="faq-item"
								itemscope
								itemprop="mainEntity"
...
<style>
	...
	.p-file-card:hover .p-card-remove {
		opacity: 1;
	}
	.p-card-remove:hover {
		background: #d94040;
	}
	.p-card-remove:disabled {
		cursor: default;
	}
	...
	.p-viewer-scroll::-webkit-scrollbar {
		width: 6px;
	}
	.p-viewer-scroll::-webkit-scrollbar-thumb {
		background: var(--border);
		border-radius: 3px;
	}
	.p-viewer-pages {
